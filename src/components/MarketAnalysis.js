...
async function runAnalysis(container, pair) {
  ...
  try {
    ...
    if (!candles || candles.length < 30) {
      signalSection.innerHTML = `<div style="text-align:center;color:var(--text-tertiary);">Insufficient data</div>`;
      return;
    }
    ...
    signalSection.innerHTML = `
      <div style="text-align:center;padding:var(--space-3);background:${sc.bg};border-radius:var(--radius-md);">
        <div style="font-size:1.5rem;margin-bottom:var(--space-1);">${sc.icon}</div>
        <div style="font-size:var(--text-lg);font-weight:var(--font-extrabold);color:${sc.color};">${sc.label}</div>
